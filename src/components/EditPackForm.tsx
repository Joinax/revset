...
export default function EditPackForm({ packId, categories, approvedProducts, onSuccess, onCancel }: Props) {
  ...
  const autoImages = selectedIds
    ...
    .filter((p): p is typeof approvedProducts[number] => !!p && p.images.length > 0)
    ...
    <form onSubmit={handleSubmit} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', display: 'grid', gap: '20px' }}>
      ...
      <div>
        <label style={labelStyle}>
          Карточки * — выбрано {selectedIds.length}
        ...
        {approvedProducts.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '260px', overflowY: 'auto', padding: '4px' }}>
            {approvedProducts.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => toggleProduct(p.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '10px', cursor: 'pointer', border: `1px solid ${sel ? 'var(--accent)' : 'var(--border)'}`, background: sel ? 'rgba(72,128,255,0.07)' : 'var(--bg)', transition: 'background 0.15s' }}>
                  ...
                  <div style={{ width: '18px', height: '18px', borderRadius: '50%', border: `2px solid ${sel ? 'var(--accent)' : 'var(--border)'}`, background: sel ? 'var(--accent)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {sel && <i className="ti ti-check" style={{ fontSize: '11px', color: '#fff' }} />}
                  </div>
                </div>
              )
            })}
