...
export const metadata: Metadata = {
  title: 'О нас | REVSET',
  description: 'REVSET — российский маркетплейс BIM-семейств для Autodesk Revit. Мы объединяем авторов и BIM-специалистов.',
}
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <section style={{ padding: '48px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1px', background: 'var(--border)', maxWidth: '640px', margin: '0 auto', border: '1px solid var(--border)', borderRadius: '14px', overflow: 'hidden' }}>
          {STATS.map(stat => (
            <div key={stat.label} style={{ background: 'var(--bg2)', padding: '24px', textAlign: 'center' }}>
      ...
      <section style={{ padding: '0 24px 48px', maxWidth: '640px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
          {VALUES.map(v => (
            <div key={v.title} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '18px' }}>
              <i className={`ti ${v.icon}`} style={{ fontSize: '22px', color: 'var(--accent)', display: 'block', marginBottom: '10px' }} />
